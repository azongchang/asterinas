...
impl InodeBlockManager {
    ...
    pub fn write_blocks_async(
        ...
        for dev_range in DeviceRangeReader::new(self, bid..bid + nblocks as Ext2Bid)? {
            ...
            self.fs().write_blocks_async_journaled(
                start_bid,
                bio_segment,
                self.write_kind(),
                None,
...
impl InodeBlockManager {
    fn write_kind(&self) -> Ext2WriteKind {
        let Some(inode) = self.weak_inode.upgrade() else {
            return Ext2WriteKind::Metadata;
        };
        match inode.inode_type() {
            InodeType::File | InodeType::SymLink => Ext2WriteKind::Data {
                ino: inode.ino() as u64,
            },
            _ => Ext2WriteKind::Metadata,
        }
    }
}
